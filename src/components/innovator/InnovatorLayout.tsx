...
export function InnovatorLayout({ children }: InnovatorLayoutProps) {
  ...
  const nav: Array<{
    ...
    { href: "/innovator", label: t.navDashboard, match: (p) => p === "/innovator" },
    { href: "/innovator/projects", label: t.navProjects, match: (p) => p.startsWith("/innovator/projects") },
    {
      href: "/innovator/messages",
      label: t.navMessages,
      match: (p) => p.startsWith("/innovator/messages"),
      badge: MESSAGE_BADGE,
    },
    { href: "/innovator/profile", label: t.navProfile, match: (p) => p.startsWith("/innovator/profile") },
  ];
      ...
      <div className="lg:pl-64">
        <header className={`sticky top-0 z-30 flex h-16 items-center gap-3 border-b px-4 ${topBar}`}>
          ...
          <div className="ml-auto flex items-center gap-3">
            <span
              className={`max-w-[10rem] truncate text-sm font-semibold sm:max-w-none ${isDark ? "text-zinc-200" : "text-zinc-800"}`}
              title={userDisplay}
            >
              {userDisplay}
            </span>
